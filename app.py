...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:'+DB_PASS+'@localhost/lab5'
...
def newpost():
    ...
    if request.method == 'POST':
        ...
        # Create in DB
        new_post = Post(author=session_user.uid, content=content, date=datetime.date(datetime.now()))
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if existing_user:
            flash('The username already exists. Please pick another one.', 'error')
